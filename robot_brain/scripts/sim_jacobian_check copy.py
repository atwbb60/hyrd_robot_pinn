...
# ================= 2. 网络架构 (从 train_topo_impulse_big.py 1:1 搬运) =================
...
class TopoImpulseNet(nn.Module):
    def __init__(self, scalers, input_dim=15, feature_dim=128, gru_dim=128, dropout=0.2):
        ...
        # ⚠️ 关键修正：head_input_dim = 128 + (128 * 2) + 6 = 390
    ...
    def forward(self, x_inputs, jacobian_norm, dq_cmd_norm, return_internals=True, debug_mode=False):
        """
        前向传播函数
        :param x_inputs: 输入特征向量 [B, 5, 15]
        :param jacobian_norm: 归一化的局部雅可比 [B, 5, 6]
        :param dq_cmd_norm: 归一化的电机指令 [B, 5, 2]
        :param return_internals: 是否返回 beta 和标称值等中间变量
        :param debug_mode: 是否开启调试模式（True: 仅输出物理标称值；False: 输出混合预测值）
        """
        B = x_inputs.shape[0]
        # Stage 1: 提取局部特征
        ...
        for i in range(self.num_nodes):
            ...
            if debug_mode:
                # 调试模式：强制输出物理标称值
                outputs.append(nominal_delta[:, i, :])
            else:
                # 正常模式：物理先验与神经网络混合预测
                outputs.append(beta * nominal_delta[:, i, :] + (1.0 - beta) * net_prediction)
...
class HybridPhantomSimV11:
    ...
    def control_loop(self):
        rob_poses, rob_segs = forward_kinematics_full_chain(self.q_robot, return_arc_points=True)
        targ_poses, targ_segs = forward_kinematics_full_chain(self.q_target, return_arc_points=True)
        
        # 1. 全局解析雅可比 (10x10) - 用于基准
        eps = 1e-3; J_phy = np.zeros((10, 10))
        base_xy = rob_poses[:, :2].flatten()
        for i in range(10):
        ...
        if self.use_neural:
            ...
            p_l = []
            prev_p_np = np.array([0.0, 0.0, 0.0])
            for i in range(5):
                curr_p_np = rob_poses[i]
                th_p = prev_p_np[2]; dx, dy = curr_p_np[0]-prev_p_np[0], curr_p_np[1]-prev_p_np[1]
                lx, ly = dx*np.cos(th_p)+dy*np.sin(th_p), -dx*np.sin(th_p)+dy*np.cos(th_p)
                p_l.append([lx, ly, curr_p_np[2]-prev_p_np[2]])
                prev_p_np = curr_p_np
            p_l_t = norm(np.array(p_l), 'pose_loc').to(self.device).unsqueeze(0)
            
            dq_cmd_t = torch.zeros((1, 5, 2), requires_grad=True, device=self.device)
            ...
            # --- 核心：Differentiable FK 链式连接 ---
            # 1. 获取网络预测的局部增量 (当前强制输出标称值)
            x_in = torch.cat([q_t, dq_h, p_l_t, dq_cmd_t, j_f_t], dim=2)
            pred_local, _, beta = self.model(x_in, j_f_t, dq_cmd_t, return_internals=True)
            
            # 2. 构造预测的局部下一状态
            current_local_raw = torch.from_numpy(np.array(p_l)).float().to(self.device).unsqueeze(0)
            next_local = current_local_raw + pred_local # [1, 5, 3]
            
            # 3. 链式相乘实现全局坐标输出 (1:1 搬运 DifferentiableFK 逻辑)
            T_curr = torch.eye(3, device=self.device).unsqueeze(0)
            global_points = []
            for i in range(5):
                lx, ly, lth = next_local[0, i, 0], next_local[0, i, 1], next_local[0, i, 2]
                cos_t, sin_t = torch.cos(lth), torch.sin(lth)
                # 构造局部变换矩阵 T
                r0 = torch.stack([cos_t, -sin_t, lx])
                r1 = torch.stack([sin_t, cos_t, ly])
                r2 = torch.tensor([0.0, 0.0, 1.0], device=self.device)
                T_local = torch.stack([r0, r1, r2]).unsqueeze(0)
                
                T_curr = torch.bmm(T_curr, T_local)
                global_points.append(T_curr[0, 0, 2]) # Global X
                global_points.append(T_curr[0, 1, 2]) # Global Y
            
            # 展平为 10 维全局坐标向量
            full_global_xy = torch.stack(global_points) 
            
            # 4. 🔥 计算全局雅可比：对全链条输出求导
            dq_std_raw = self.scalers['dq_cmd_std'].cpu().numpy()
            dq_std_phys = np.tile(dq_std_raw, (5, 1)) 
            
            J_net_global = np.zeros((10, 10))
            for i in range(10):
                # 自动微分捕捉了 joint_i 对 node_j (j>=i) 的所有拓扑影响
                grad = torch.autograd.grad(full_global_xy[i], dq_cmd_t, retain_graph=True)[0]
                grad_phys = grad[0].cpu().numpy() / dq_std_phys
                J_net_global[i, :] = grad_phys.flatten()
            
            beta_val = beta[0, 4, 0].item()
            curr_J = beta_val * J_phy + (1.0 - beta_val) * J_net_global

        # --- 稳定性控制 ---
        error = targ_poses[:, :2].flatten() - base_xy
        dq = np.linalg.inv(curr_J.T @ curr_J + 0.5 * np.eye(10)) @ curr_J.T @ (error * 0.15)
        ...
        self.pt_selected.set_data([targ_poses[self.selected_idx, 0]], [targ_poses[self.selected_idx, 1]])
        self.ax.set_title(f"V11.4 Global Jacobian | Mode: {'NEURAL' if self.use_neural else 'PHY'} | Beta: {beta_val:.2f}")
